...
- `measure_batch_gamma_sweep.rs` — batch-vs-loop benchmark for the ChebCode
  η-sweep workflow.
- `measure_cheb_sweep.rs` / `measure_xtreme_duel.rs` / `measure_fft5_duel.rs` /
  `measure_build_share.rs` / `measure_windowed_ratio.rs` — Pareto-campaign
  probes (ChebCode preset grid, preset duels, the speed_seq p≤50000 dispatch
  bin re-check, tree-build share, windowed-ratio curve).
- `measure_pipeline_phases.rs` — phase breakdown of `deconvolve_spiked` at
  p=20k (tree build vs evaluation vs deconvolution).
